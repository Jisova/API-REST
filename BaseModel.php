...
/**
 * La clase BaseModel proporciona una serie de caracteristicas convenientes que
 * hace que trabajar con bases de datos sea menos doloroso. Ampliando esta clase
 * proporcionar medios para implementar varios sistemas de bases de datos
 *
 * Va a:
...
{
    /**
     * Instancia de buscapersonas.
     * Completado despues de llamar $this->paginate()
     *
     * @var Pager
     */
    public $pager;
    ...
    /**
     * El grupo de conexion de base de datos que
    ...
    /**
     * El formato con el que se deben devolver los resultados.
     * Se anulara si se utilizan los metodos as*.
     *
     * @var string
     */
    ...
    /**
     *Si este modelo debe usar "softDeletes" y
     * simplemente establezca una fecha en la que se eliminen las filas, o
     * hacer eliminaciones completas.
     *
     * @var bool
     */
    ...
    /**
     *Deberian eliminarse las reglas de validacion antes de guardar?
     * Mas util al realizar actualizaciones.
     *
    ...
    /**
     * Validar los datos con las reglas de validacion (o el grupo de validacion)
     * especificado en la propiedad de clase, $validationRules.
     *
     * @param array|object $data Data
     */
    ...
    /**
     * Devuelve las reglas de validacion definidas del modelo para que
     * se puede utilizar en otro lugar, si es necesario.
     *
    ...
    public function getValidationRules(array $options = []): array
    {
        ...
        // ValidationRules puede ser una cadena, que es el nombre del grupo,
        // o una serie de reglas.
        if (is_string($rules)) {
    ...
    /**
     * Devuelve los mensajes de validacion definidos del modelo para que
     * se puede utilizar en otro lugar, si es necesario.
     */
    ...
    /**
     *Elimina cualquier regla que se aplique a campos que no se han configurado
     * actualmente para que las reglas no bloqueen la actualizacion cuando solo se actualiza
     * una fila parcial.
     *
    ...
    /**
     * Establece el valor de $tempAllowCallbacks para que podamos anularlo temporalmente
     * el ajuste. Se reinicia despues del siguiente metodo que utiliza activadores.
     *
     * @param bool $val value
     *
     * @return $this
    ...
    /**
     *Un activador de evento simple para eventos modelo que permite
     * manipulacion de datos dentro del modelo. Disenado especificamente para
     * uso por modelos secundarios, esto se puede usar para formatear datos,
     * guardar/cargar clases relacionadas, etc.
     *
     * Es responsabilidad de los metodos de devolucion de llamada devolver
     * los datos en si.
     *
     * Cada matriz $eventData DEBE tener una clave de 'datos' con la informacion relevante
     * datos para metodos de devolucion de llamada (como una matriz de pares clave/valor para insertar
     * o actualizar, una serie de resultados, etc.)
     *
     * Si no se permiten devoluciones de llamada, devuelve $eventData inmediatamente.
     *
    ...
    {
        // Asegurese de que sea un evento valido
    ...
    /**
     * Establece el tipo de retorno de los resultados como una matriz asociativa.
     *
     * @return $this
     */
    ...
    /**
     *Establece el tipo de retorno para que sea del tipo de objeto especificado.
     * El valor predeterminado es un objeto simple, pero puede ser cualquier clase que tenga
     * vars de clase con el mismo nombre que las columnas de la coleccion,
     * o al menos permite crearlos.
     *
    ...
    /**
     * Toma una clase y devuelve una serie de datos publicos y protegidos.
     * propiedades como una matriz adecuada para usar en creaciones y actualizaciones.
     * Este metodo utiliza objectToRawArray internamente y realiza la conversion.
     * para encadenar en todas las instancias de tiempo
     *
    ...
    protected function objectToArray($data, bool $onlyChanged = true, bool $recursive = false): array
    {
        ...
        // Convierta cualquier instancia de Time al formato $dateFormat apropiado
        if ($properties) {
    ...
    /**
     *Toma una clase y devuelve una serie de datos publicos y protegidos.
     * propiedades como una matriz con valores sin procesar.
     *
     * @param object|string $datos         datos
     * @param bool          $onlyChanged   Solo propiedad cambiada
     * @param bool          $recursivo     Si es verdadero, las entidades internas tambien se convertiran en una matriz
     *
     * @throws ReflectionException
     *
    ...
    protected function objectToRawArray($data, bool $onlyChanged = true, bool $recursive = false): ?array
    {
        if (method_exists($data, 'toRawArray')) {
            ...
            // guardando el nombre/valor en una nueva matriz que podemos devolver.
    ...
    protected function transformDataToArray($data, string $type): array
    {
        ...
        // Si $data esta usando una clase personalizada con publico o protegido
        // propiedades que representan los elementos de la coleccion, debemos tomarlas
        // ellos como una matriz.
        if (is_object($data) && ! $data instanceof stdClass) {
            $data = $this->objectToArray($data, ($type === 'update'), true);
        }

        // Si todavia es una stdClass, continua y conviertela a
        // una matriz para que doProtectFields y otros metodos del modelo
        // no es necesario realizar comprobaciones especiales.
        if (is_object($data)) {
            $data = (array) $data;
        }

        // Si todavia est avacio aqui, significa que $data no ha cambiado o es un objeto vacio.
        if (empty($data)) {
            throw DataException::forEmptyDataset($type);
        }
    ...
    /**
     * Proporciona la conexion de base de datos y las propiedades del modelo.
     *
    ...
    /**
     * Comprueba la existencia de propiedades en este modelo y la conexion de base de datos.
     *
     * @param string $name Name
     */
    ...
    /**
     *Proporciona acceso directo al metodo en la conexion de la base de datos.
     *
     * @param string $name   Name
    ...
    /**
     * Reemplace los marcadores de posicion dentro de las reglas con los valores que
     * coincide con la 'clave' de cualquier propiedad que se establezca. Por ejemplo, si
     * teniamos la siguiente matriz $data:
     *
     * ['identificacion' => 13]
     *
     * y la siguiente regla:
     *
     * 'requerido|es_unique[usuarios,correo electronico,id,{id}]'
     *
     * El valor de {id} se reemplazaria con la identificacion real en los datos del formulario:
     *
